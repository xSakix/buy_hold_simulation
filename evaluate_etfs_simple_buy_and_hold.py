...
result_csv = 'evaluation_results/' + prefix + 'evaluation_result_10.csv'
...
for ticket in etf_list:
    ...
    while i < MAX_ITER:
        ...
        bah.invest(data[ticket])
        if failed > MAX_ITER:
            break
        if bah.investor.invested == 0. or np.isnan(bah.investor.returns):
            failed += 1
            continue
        # print('value invested:' + str(bah.investor.invested))
        # print('value gained:' + str(bah.investor.gains))
        # print('returns:' + str(bah.investor.returns))
        returns[i] = bah.investor.returns
        i += 1
